...
def getData(arguments, requests_results):
    ...
    data["domain_or_domains"] = manage_plural(data["nb_domains"], ("Domain", "Domains"))
...
def render(
    ...
    data["boolean_azure"] = "inline" if arguments.boolean_azure else "none"
    ...
    for category_repartition in ["on_premise", "azure"]:
        ...
        data[category_repartition]["issue_or_issues"] = manage_plural(
            data[category_repartition]["immediate_risk"], ("issue", "issues")
        )
        data[category_repartition]["vuln_text_major_risk"] = manage_plural(
            data[category_repartition]["potential_risk"],
            ("vulnerability", "vulnerabilities"),
        )
        data[category_repartition]["alert_or_alerts"] = manage_plural(
            data[category_repartition]["potential_risk"], ("Alert", "Alerts")
        )
        data[category_repartition]["minor_alert_or_alerts"] = manage_plural(
            data[category_repartition]["minor_risk"], ("Minor issue", "Minor issues")
        )
